...
class ParticleScene extends Phaser.Scene {
  ...
  /**
   * 在指定位置播放粒子效果
   * @param {string} type - 效果类型: water, harvest, plant, levelup, gold, exp
   * @param {number} x - X坐标
   * @param {number} y - Y坐标
   */
  emitEffect(type, x, y) {
  ...
  _emitHarvest(x, y) {
    ...
    const goldEmitter = this.add.particles(x, y, 'gold_coin', {
      ...
      quantity: 15,
      gravityY: 100,
      tint: [0xffd700, 0xffc107, 0xffeb3b],
      emitting: false,
    });
  ...
  _emitPlant(x, y) {
    ...
    const dirtEmitter = this.add.particles(x, y + 10, 'dirt', {
      ...
      quantity: 10,
      gravityY: 120,
      tint: [0x8B4513, 0xa0522d, 0x6b3a1f],
      emitting: false,
    });
  ...
  _emitExp(x, y) {
    const emitter = this.add.particles(x, y, 'exp_orb', {
      ...
      quantity: 10,
      gravityY: -50,
      tint: [0x7c4dff, 0xb388ff, 0x448aff],
      emitting: false,
    });
    emitter.explode(10);
  ...
  return {
    ...
    emitEffect(type, screenX, screenY) {
      if (scene && scene.scene.isActive()) {
        scene.emitEffect(type, screenX, screenY);
      }
    },
    destroy() {
